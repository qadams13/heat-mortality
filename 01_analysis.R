...
DATE1_CALI = as.Date( "2018-01-06" );
DATE2_CALI = as.Date( "2020-01-02" );

# Period for the Predictions: 2015-2022
DATE1_PRED = as.Date( "2018-01-06" );
DATE2_PRED = as.Date( "2023-12-30" );

# Period for Summer 2023
DATE1_SU22 = as.Date( "2023-05-06" );
DATE2_SU22 = as.Date( "2023-09-30" );

# Period for Summer 2024
# DATE1_SU22 = as.Date( "2024-05-06" );
# DATE2_SU22 = as.Date( "2024-09-30" );
...
# if( wday( DATE1_SU22, week_start = 1 ) != 4 | wday( DATE2_SU22, week_start = 1 ) != 4 ){ stop("ERROR: Invalid Summer 2023 Period !!!"); }
# 
...
DATATABLE <- readRDS("/restricted/projectnb/climlab/CDC_Heat_Mortality/heat_mortality_500k.RDS")

# For Convenience, We Identify the Date of Each Week with Its Thursday
# DATATABLE$date2 = ISOweek2date( paste0( DATATABLE$year, "-W", sprintf( "%02d", DATATABLE$woy ), "-", 4 ) );
DATATABLE <- DATATABLE %>%
  rename(date = X_week_end_date)

DATATABLE$pandemic <- ifelse(DATATABLE$year == "2020", 1,0)
...
vREG = unique(DATATABLE$StCoFIPS);
nREG = length(vREG);
...
DATATABLE_CALI = DATATABLE[ which( DATE1_CALI <= DATATABLE$date & DATATABLE$date <= DATE2_CALI +  MAX_LAG ), ];
DATATABLE_PRED = DATATABLE[ which( DATE1_PRED <= DATATABLE$date & DATATABLE$date <= DATE2_PRED +  MAX_LAG ), ];
# rm(DATATABLE);

# Creating the Datalists of the Calibration and Prediction Periods
DATALIST_CALI = lapply( vREG, function(x) DATATABLE_CALI[ DATATABLE_CALI$StCoFIPS == x, ] ); names(DATALIST_CALI) = vREG;
# DATALIST_CALI = keep(DATALIST_CALI, ~all(nrow(.x) == 157))

DATALIST_PRED = lapply( vREG, function(x) DATATABLE_PRED[ DATATABLE_PRED$StCoFIPS == x, ] ); names(DATALIST_PRED) = vREG;
# DATALIST_PRED = keep(DATALIST_PRED, ~all(nrow(.x) == 313))


# cali <- names(DATALIST_CALI)
# #finding which one is the outlier - 48329
# setdiff(cali, pred) 
# 
# # #getting rid of 17093 from DATALIST_CALI
# DATALIST_CALI = DATALIST_CALI[names(DATALIST_CALI) != "17093"]
# DATALIST_PRED = DATALIST_PRED[names(DATALIST_PRED) != "17093"]
...
# ranges <- t(sapply(DATALIST_CALI, function(x) range(x$mean_temp_C,na.rm=T)))
# bound <- colMeans(ranges)
# varknots <- equalknots(bound,fun="bs",degree=2,df=4)
# lagknots <- logknots(3,df=5,int=T)
...
for( i in 1:nREG ){
  ...
  if( VAR_FUN == "ns" ){
    CROSS_BASIS = crossbasis(sub$mean_temp_C,
                              c( MIN_LAG, MAX_LAG ),
                              argvar = list( fun = VAR_FUN,
                                             knots = quantile( sub$mean_temp_C, VAR_PRC, na.rm = TRUE ),
                                             Boundary.knots = range( sub$mean_temp_C, na.rm = TRUE ) ),
  ...
  GLM_MODEL = glm( formula = X_deaths ~ ns( wop, df = round( DF_SEAS * length(wop) * 7 / 365.25 ) ) + pandemic + CROSS_BASIS,
                   sub,
                   family = quasipoisson,
                   na.action = "na.exclude" );
  
  # Cumulative Exposure-Response without Centring (Before the Meta-Analysis)
  CROSS_PRED_REG_NOMETA <- crosspred( CROSS_BASIS, GLM_MODEL, at = quantile(sub$mean_temp_C, PRED_PRC, na.rm = TRUE ) ) ;
...
TEMP_AVG = sapply( DATALIST_CALI, function(x) mean( x$mean_temp_C, na.rm = TRUE ) );
TEMP_IQR = sapply( DATALIST_CALI, function(x)  IQR( x$mean_temp_C, na.rm = TRUE ) );
...
for( iREG in 1:nREG ){
  ...
  # Temperature Basis
  if( VAR_FUN == "ns" ){
    BASIS_VAR = onebasis( quantile( DATALIST_CALI[[iREG]]$mean_temp_C, PRED_PRC, na.rm = TRUE ),
                          fun = VAR_FUN,
                          knots = quantile( DATALIST_CALI[[iREG]]$mean_temp_C, VAR_PRC, na.rm = TRUE ),
                          Boundary.knots = range( DATALIST_CALI[[iREG]]$mean_temp_C, na.rm = TRUE ) );
  }else if( VAR_FUN == "bs" ){
    BASIS_VAR = onebasis( quantile( DATALIST_CALI[[iREG]]$mean_temp_C, PRED_PRC, na.rm = TRUE ),
                          fun = VAR_FUN,
                          degree = VAR_DEG,
  ...
  PRC_VAR = quantile( DATALIST_CALI[[iREG]]$mean_temp_C, PRED_PRC, na.rm = TRUE );
...
for( iREG in 1:nREG ){
  TMIN[iREG] =      min( DATALIST_PRED[[iREG]]$mean_temp_C,        na.rm = TRUE );
  P025[iREG] = quantile( DATALIST_PRED[[iREG]]$mean_temp_C, 0.025, na.rm = TRUE );
  P975[iREG] = quantile( DATALIST_PRED[[iREG]]$mean_temp_C, 0.975, na.rm = TRUE );
  TMAX[iREG] =      max( DATALIST_PRED[[iREG]]$mean_temp_C,        na.rm = TRUE );
}
rm(iREG);
...
for( iREG in 1:nREG ){
  ...
  # Temperature Basis
  if( VAR_FUN == "ns" ){
    BASIS_VAR = onebasis( DATALIST_PRED[[iREG]]$mean_temp_C,
                          fun = VAR_FUN,
                          knots = quantile( DATALIST_CALI[[iREG]]$mean_temp_C, VAR_PRC, na.rm = TRUE ),
                          Boundary.knots = range( DATALIST_CALI[[iREG]]$mean_temp_C, na.rm = TRUE ) );
  ...
  LAGGED_MORT_MATRIX = Lag( DATALIST_PRED[[iREG]]$X_deaths, -MIN_LAG:-MAX_LAG );
  ...
  for( iPER in 1:length(vPER_PRED) ){
    ...
    else if( vPER_PRED[iPER] == "Summer 2023" ){ vTIM = which( DATE1_SU22 <= DATALIST_PRED[[iREG]]$date & DATALIST_PRED[[iREG]]$date <= DATE2_SU22 ); } # Summer 2023
    ...
    MORT_REG[iREG,iPER] =  sum( DATALIST_PRED[[iREG]]$X_deaths[vTIM], na.rm = TRUE );
    POPU_REG[iREG,iPER] = mean( DATALIST_PRED[[iREG]]$population_estimate[vTIM], na.rm = TRUE );
    ...
    for( iRNG in 1:nRNG ){
      ...
      else if( vRNG[iRNG] == "Total Cold"    ){ vRNG_THRES = which(                                                     DATALIST_PRED[[iREG]]$mean_temp_C[vTIM] <  MMT_REG[iREG] ); } # Total Cold
      else if( vRNG[iRNG] == "Total Heat"    ){ vRNG_THRES = which( MMT_REG[iREG] <  DATALIST_PRED[[iREG]]$mean_temp_C[vTIM]                                                     ); } # Total Heat
      else if( vRNG[iRNG] == "Moderate Cold" ){ vRNG_THRES = which(    P025[iREG] <= DATALIST_PRED[[iREG]]$mean_temp_C[vTIM] & DATALIST_PRED[[iREG]]$mean_temp_C[vTIM] <  MMT_REG[iREG] ); } # Moderate Cold
      else if( vRNG[iRNG] == "Moderate Heat" ){ vRNG_THRES = which( MMT_REG[iREG] <  DATALIST_PRED[[iREG]]$mean_temp_C[vTIM] & DATALIST_PRED[[iREG]]$mean_temp_C[vTIM] <=    P975[iREG] ); } # Moderate Heat
      else if( vRNG[iRNG] == "Extreme Cold"  ){ vRNG_THRES = which(                                                     DATALIST_PRED[[iREG]]$mean_temp_C[vTIM] <     P025[iREG] ); } # Extreme Cold
      else if( vRNG[iRNG] == "Extreme Heat"  ){ vRNG_THRES = which(    P975[iREG] <  DATALIST_PRED[[iREG]]$mean_temp_C[vTIM]                                                     ); } # Extreme Heat
...
plot(as.Date( vPER_PRED[1:nPER_PRED] ), 
            AN_TOT[1,1:nPER_PRED,"Total Cold","att_val"],
            col = rgb(0.00,0.00,1.00    ), lwd = 2, lty = 1, type = "l", main = "Temperature Related Mortality", xlab = "Time (week)", ylab = "Mortality (weekly deaths)", xaxt = "n" );
polygon( c( as.Date( vPER_PRED[1:nPER_PRED] ), rev( as.Date( vPER_PRED[1:nPER_PRED] ) ) ),
         c( AN_TOT[1,1:nPER_PRED,"Total Cold","att_low"], rev( AN_TOT[1,1:nPER_PRED,"Total Cold","att_upp"] ) ), col = rgb(0.00,0.00,1.00,1/3), border = FALSE );
...
axis   ( 1, at = as.Date( c("2018-07-01", "2019-07-01", "2020-07-01", "2021-07-01", "2022-07-01", "2023-07-01" ) ),
...
plot(CROSS_PRED_REG_META$allRRfit, type = "n", ylab = "RR", ylim = c(0.8, 2))
for(i in seq(nREG)) lines(nREG[[i]], ptype = "overall", col = grey(0.5), lty = 2)
abline(h = 1)
lines()


# Cumulative Exposure-Response (After the Meta-Analysis)
CROSS_PRED_REG_META_all = crosspred( BASIS_VAR,
                                         coef = BLUP[[iREG]]$blup,
                                         vcov = BLUP[[iREG]]$vcov,
                                         model.link = "log",
                                         at = PRC_VAR,
